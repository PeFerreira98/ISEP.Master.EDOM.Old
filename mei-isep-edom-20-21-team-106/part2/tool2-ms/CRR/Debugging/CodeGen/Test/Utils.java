...
public class Utils
{
    ...
    public static void displayItems(){
        for (var prod : TestModel.Products) {
    ...
    private static Product getProductById(int id)
    {
        ...
        for (var otherItem : TestModel.Products)
        {
            if (otherItem.Id == id)
            {
                item = otherItem;
    ...
    public static void createComment(IUser user)
    {
        ...
        try
        {
            item = getProductById(Integer.parseInt(itemId));
        } 
        catch (Exception e)
        {
            // do nothing
        ...
        comment.Author = user;
        comment.Approved = ApprovalComment.initApprovalComment(comment);
    ...
    public static void createRate(IUser user)
    {
        ...
        rate.Author = user;
        rate.Approved = RateApproval.initRateApproval(rate);
        item.Rates.add(rate);
    }
}
