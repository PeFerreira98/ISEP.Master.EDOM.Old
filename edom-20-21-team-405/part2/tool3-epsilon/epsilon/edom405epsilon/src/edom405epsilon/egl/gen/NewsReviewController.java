...
public class NewsReviewController{
	...
	public List<Comment> checkComments(String user, String text, LocalDateTime date,
	 List<Comment> comments) {
        ...
        for(int i= 0; i < comments.size(); i++) {
            NewsReview testComment = (NewsReview)comments.get(i);
    ...
    public static void listcomments(Model model, String subjectID, Scanner scanner){
        ...
        for(int i = 0; i < comments.size(); i++){
            System.out.println( i+1 + ". " + ((NewsReview)comments.get(i)).gettext());
        ...
        if(optionNumber >= 1 && optionNumber <= comments.size()){
            ...
            if(com.getcomments().size() > 0){
                System.out.println("-------------------\n");
                System.out.println("Comments From Comment '" + ((NewsReview)com).gettext() + "': \n");
                for(int i = 0; i < com.getcomments().size(); i++){
                    System.out.println( i+1 + ". " + ((NewsReview)com.getcomments().get(i)).gettext());

                }
                System.out.println("-------------------\n");
            }
            System.out.println("========================\n");
            NewsController.subjectDataComments(model, subjectID, scanner);         
